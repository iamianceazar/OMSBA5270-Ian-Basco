...
revenue_dataframe = pd.DataFrame.from_dict((companyFacts.json()['facts']['us-gaap']['Revenues']['units']['USD']))
# print(assets_dataframe)

revenue_dataframe = revenue_dataframe[revenue_dataframe.frame.notna()]
# print(assets_dataframe)
...
revenue_dataframe_filtered = revenue_dataframe[revenue_dataframe['form'] == '10-K']
liabilities_dataframe_filtered = liabilities_dataframe[liabilities_dataframe['form'] == '10-K']
print(revenue_dataframe_filtered, '\n', liabilities_dataframe_filtered)
#
# merge assets and liabilities dataframes
merge_dataframes = pd.merge(revenue_dataframe_filtered, liabilities_dataframe_filtered, on ='accn')
print(merge_dataframes)
#
merge_dataframes = merge_dataframes.drop(merge_dataframes.columns[12:], axis = 1)
print(merge_dataframes)
#
# calculating ratio of revenue to liabilities (debt ratio)
merge_dataframes['ratio_revenue_liabilities'] = merge_dataframes['val_x'] / merge_dataframes['val_y']
print(merge_dataframes)

# rename the columns

merge_dataframes = merge_dataframes.rename(columns={'val_x' : 'revenues', 'val_y' : 'liabilities'})
print(merge_dataframes)

# import plotly.express package and graph the data
pd.options.plotting.backend = "plotly"
graph = merge_dataframes.plot(x = "filed_x",
                              y = ['revenues', 'liabilities'],
                              title = companyTitle + " ratio of revenues and liabilities",
                              labels={"filed_x": "Quarter End Data",
                                      "ratio_revenue_liabilities": "Value in dollars ($)"}
                              )
graph.show()